...
function allocate(db,meal,date){const members=db.members; const tasks=normalizedSlots(db); const history={}; members.forEach(m=>history[m.id]=0);
  ...
  sameDay.forEach(a=>a.items.forEach(i=>{if(history[i.memberId]!==undefined)history[i.memberId]+=i.normalizedWeight}));
...
app.post('/api/assign',(req,res)=>{const db=load();const meal=req.body.meal,date=req.body.date||new Date().toISOString().slice(0,10);if(!['breakfast','lunch','dinner'].includes(meal))return res.status(400).json({error:'Invalid meal'});if(db.members.length<1)return res.status(400).json({error:'At least one member is required'});db.assignments=db.assignments.filter(a=>!(a.date===date&&a.meal===meal));const items=allocate(db,meal,date);db.assignments.push({id:'a'+Date.now(),date,meal,items,createdAt:new Date().toISOString()});save(db);res.json(db);});
